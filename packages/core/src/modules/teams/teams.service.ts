...
import { PlanTier } from "../../database/entities/subscription.entity";
...
export class TeamsService {
  ...
  async create(
    ...
    this.logger.log(
      `Team created: ${saved.name} (${saved.slug}) by ${ownerId}`,
    );
    return saved;
  }
  ...
  /**
   * Get or create the personal (default) team for a user.
   * Every user gets one personal team that cannot be deleted.
   */
  async getOrCreatePersonalTeam(userId: string): Promise<Team> {
    ...
    let slug = baseSlug;
    let attempt = 0;
    while (await this.teamRepo.findOne({ where: { slug } })) {
      attempt++;
    ...
    this.logger.log(`Personal team created: ${saved.slug} for user ${userId}`);
    return saved;
  }

  /**
   * Get a user's membership in a team (for guards).
   */
  async getMembership(
    teamId: string,
    userId: string,
  ...
  async delete(teamId: string, userId: string): Promise<void> {
    ...
    const team = await this.findById(teamId);
    if (team.isPersonal) {
      throw new ForbiddenException("Cannot delete your personal team");
    }
  ...
  async inviteMember(
    ...
    this.logger.log(
      `${email} invited to team ${teamId} as ${role} by ${inviterId}`,
    );
    return saved;
  }
  ...
  async removeMember(
    ...
    this.logger.log(
      `Member ${memberId} removed from team ${teamId} by ${removerId}`,
    );
  }
