...
export class WebhooksService {
  ...
  async findByTeam(teamId: string): Promise<Webhook[]> {
    return this.webhookRepo.find({
      where: { teamId },
      order: { createdAt: "DESC" },
    });
  ...
  async dispatch(event: string, payload: any, applicationId?: string) {
    ...
    const matching = webhooks.filter((w) => {
      if (w.applicationId && applicationId && w.applicationId !== applicationId)
        return false;
    ...
    if (matching.length > 0) {
      this.logger.log(
        `Webhook dispatch: ${event} → ${delivered} delivered, ${failed} failed`,
      );
    }
  }
  ...
  async test(
    ...
    const hook = await this.findById(id, ownerId);
    try {
      await this.deliverWebhook(hook, "ping", {
        message: "Webhook test from Agentbase",
      });
      return { success: true };
    } catch (err: any) {
