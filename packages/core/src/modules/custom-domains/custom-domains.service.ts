...
import {
  CustomDomain,
  DomainStatus,
} from "../../database/entities/custom-domain.entity";
import { AuditService } from "../audit/audit.service";

const resolveCname = promisify(resolve);
const resolveTxt = promisify(
  (hostname: string, cb: (err: any, records?: string[][]) => void) => {
...
export class CustomDomainsService {
  ...
  async create(
    ownerId: string,
    dto: { domain: string; applicationId?: string; teamId?: string },
  ) {
    const domain = dto.domain.toLowerCase().trim();

    // Validate not reserved
    if (
      RESERVED_DOMAINS.some((r) => domain === r || domain.endsWith("." + r))
    ) {
  ...
  async verify(
    ...
    try {
      ...
      const cnameValues = Array.isArray(records)
        ? records.map((r: any) => String(r).toLowerCase())
        : [];
    ...
    try {
      const txtRecords = (await resolveTxt(
        `_agentbase-verify.${domain.domain}`,
      )) as string[][];
  ...
  async list(ownerId: string, teamId?: string) {
  ...
  async update(
    id: string,
    ownerId: string,
    dto: { applicationId?: string; settings?: any },
  ) {
    const domain = await this.getOwnedDomain(id, ownerId);
    if (dto.applicationId !== undefined)
      domain.applicationId = dto.applicationId;
