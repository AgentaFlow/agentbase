    ...
    <div>
      <div className="mb-8">
        ...
        <p className="text-slate-500 mt-1">
          Here's an overview of your AI applications.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-brand-100 rounded-lg flex items-center justify-center text-lg group-hover:scale-110 transition-transform">
              📱
            </div>
        ...
        >
          ...
          <p className="text-sm text-slate-500">
            Extend your apps with community plugins
          </p>
        </Link>
        <Link
        ...
        >
          <div className="flex items-center gap-3 mb-2">
            <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center text-lg group-hover:scale-110 transition-transform">
              ⚙️
            </div>
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-white rounded-xl border p-4 animate-pulse h-16"
              />
            ))}
          </div>
        ) : apps.length === 0 ? (
          <div className="bg-white rounded-xl border p-8 text-center">
            ...
            <Link
              href="/dashboard/applications"
              className="bg-brand-600 text-white px-5 py-2 rounded-lg hover:bg-brand-700 font-medium text-sm inline-block"
            >
              Create Application
            </Link>
          </div>
            ...
            {apps.slice(0, 5).map((app) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-xs px-2 py-0.5 rounded-full ${app.status === "active" ? "bg-green-100 text-green-700" : "bg-slate-100 text-slate-500"}`}
                  >
                    {app.status}
                  </span>
                </div>
