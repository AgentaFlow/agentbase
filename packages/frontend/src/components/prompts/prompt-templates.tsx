...
export default function PromptTemplates({
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await api.updatePromptTemplate(editId, {
          name: formName,
  ...
  const handleSetDefault = async (id: string) => {
    try {
      await api.setDefaultPromptTemplate(id, applicationId);
      await loadTemplates();
    } catch (err: any) {
      alert(err.message);
    }
    ...
    <div>
      <div className="flex items-center justify-between mb-4">
        <p className="text-sm text-slate-500">
          Manage reusable prompt templates with {"{{variable}}"} support.
        </p>
        <button
          onClick={() => {
            resetForm();
          ...
          <div className="space-y-3">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="You are a helpful {{role}} for {{company_name}}."
              ...
              {formTemplate && (
                <div className="mt-1 text-xs text-slate-400">
                  Variables:{" "}
                  {extractVariables(formTemplate).join(", ") || "none"}
                </div>
              )}
            </div>
            <div className="flex gap-2 pt-1">
              <button
                onClick={handleSave}
                disabled={!formName.trim() || !formTemplate.trim() || saving}
      ...
      {templates.length === 0 && !showCreate ? (
        <div className="text-center py-12 text-slate-400">
          ...
          <p className="text-xs mt-1">
            Create templates with variables for quick prompt setup
          </p>
        </div>
      ) : (
        <div className="space-y-3">
            ...
            <div key={t._id} className="bg-white border rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  {onSelectPrompt && (
                    <button
                      onClick={() => onSelectPrompt(t.template)}
                  ...
                  {!t.isDefault && (
                    <button
                      ...
                      className="text-xs px-2 py-1 text-slate-500 rounded hover:bg-slate-100"
                  ...
                  <button
                    onClick={() =>
                      setPreviewId(previewId === t._id ? null : t._id)
                    }
                    className="text-xs px-2 py-1 text-slate-500 rounded hover:bg-slate-100"
                  >
                    Preview
                  ...
                  <button
                    ...
                    className="text-xs px-2 py-1 text-red-500 rounded hover:bg-red-50"
                  >
                    Delete
                  </button>
                </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    {t.variables.map((v) => (
                      <input
                        ...
                        className="px-2 py-1.5 border rounded text-xs outline-none focus:ring-1 focus:ring-brand-500"
                      />
                    ))}
                  </div>
